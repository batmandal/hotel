...
const ALLOWED_TYPES: FilterType[] = ["all", "standard", "deluxe", "suite", "twin", "family"];

type ApiRoom = {
  id: string; hotelId: string; hotelName: string; hotelSlug: string; hotelStarRating: number;
  typeCode: string; typeName: string; floor: number; number: string; status: string;
  basePricePerNight: number; maxGuests: number; beds: number | null;
  amenities: string[]; sizeSqm: number | null; recommended: boolean;
  description: string | null; descriptionMn: string | null; imageUrls: string[];
};
...
const rp = (t: ReturnType<typeof useTranslations>) =>
  (t as { roomsPage?: { location: string; allLocations: string; checkIn: string; checkOut: string; guests: string; roomType: string; minHotelStars?: string; anyStars?: string; bathroom?: string; noRoomsMatch: string } }).roomsPage;
        ...
        <div className="mx-auto max-w-7xl px-4">
          ...
          <div className="mt-8 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500">{r?.checkOut ?? "Check-out"}</label>
                <Input type="date" value={checkOut} onChange={(e) => setCheckOut(e.target.value)} className="mt-1" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500">{r?.guests ?? "Guests"}</label>
            ...
            <>
              <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                {displayRooms.map((room) => (
                  <article key={room.id} className="overflow-hidden rounded-xl bg-white shadow-md transition-all duration-200 hover:shadow-lg">
                    <Link href={`/hotels/${room.hotelSlug}/rooms/${room.id}`} className="block rounded-xl focus:outline-none focus:ring-2 focus:ring-brand focus:ring-inset">
                      <div className="relative aspect-[4/3]">
                        <Image
                          src={room.imageUrls?.[0] ?? "https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=600"}
                          alt={room.typeName}
                          fill
                          className="object-cover"
                          sizes="(max-width: 768px) 100vw, 25vw"
                        />
                        <span className="absolute left-3 top-3 rounded bg-gray-800 px-2 py-1 text-xs font-medium text-white">{room.typeName}</span>
                        <button type="button" className="absolute right-3 top-3 flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-600 hover:border-brand hover:text-brand" aria-label="Save" onClick={(e) => e.preventDefault()}>
                          <Heart className="h-4 w-4" />
                        </button>
                      </div>
                      <div className="p-4">
                        <p className="font-semibold text-brand break-words">{formatPriceFromUsd(room.basePricePerNight, locale)}</p>
                        <h2 className="mt-2 font-semibold text-gray-900">{room.typeName}</h2>
                        <p className="mt-1 text-sm text-gray-600">{room.hotelName}</p>
                        <div className="mt-4 flex flex-wrap gap-4 text-sm text-gray-500">
                          <span className="flex items-center gap-1"><Bed className="h-4 w-4" aria-hidden />1 {t.common?.rooms ?? "Room"}</span>
                          <span className="flex items-center gap-1"><Bath className="h-4 w-4" aria-hidden />1 {r?.bathroom ?? (locale === "mn" ? "Угаалгын өрөө" : "Bathroom")}</span>
                          {room.sizeSqm && (<span className="flex items-center gap-1"><Square className="h-4 w-4" aria-hidden />{room.sizeSqm} m²</span>)}
                        </div>
                        <div className="mt-3 flex items-center justify-between text-sm">
                          <span className="flex items-center gap-1 text-gray-500"><MapPin className="h-4 w-4" aria-hidden />{room.hotelName}</span>
                          <span className="flex items-center gap-1 text-amber-500">★ {room.hotelStarRating}</span>
                        </div>
                      </div>
                    </Link>
                  </article>
              ...
              {filtered.length > 8 && (
                <div className="mt-12 text-center">
                  <Link href="/rooms"><Button size="lg" className="gap-2 transition-transform active:scale-[0.98]">{t.home?.viewMore ?? "View More"} <span className="text-lg">›</span></Button></Link>
                </div>
              )}
              {displayRooms.length === 0 && (
                <p className="py-12 text-center text-gray-500">{r?.noRoomsMatch ?? (locale === "mn" ? "Таны шүүлтүүрт тохирох өрөө олдсонгүй." : "No rooms match your filters.")}</p>
              )}
            </>
          )}
        </div>
    ...
    <Suspense
      fallback={
        <div className="min-h-screen bg-gray-50">
          ...
          <main className="pb-16 pt-32">
            <div className="mx-auto max-w-7xl px-4">
              <p className="text-center text-gray-500">{locale === "mn" ? "Ачаалж байна..." : "Loading..."}</p>
            </div>
          </main>
          <Footer />
        </div>
