...
export default function RoomsPage() {
  ...
  const filtered = useMemo(() => {
    return allRoomsWithHotel.filter(({ room, hotel }) => {
      ...
      if (locationId && hotel.locationId !== locationId) return false;
  ...
  }, [allRoomsWithHotel, filter, locationId, guests, checkIn, checkOut]);

  const displayRooms = filtered.slice(0, 8);
  const locationOptions = useMemo(() => {
    const ids = new Set(hotels.map((h) => h.locationId));
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
              <div>
                ...
                >
                  <option value="">
                    {r?.allLocations ??
                      (locale === "mn" ? "Бүх байршил" : "All locations")}
                  </option>
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-4 flex flex-wrap gap-4 text-sm text-gray-500">
                      ...
                      <span className="flex items-center gap-1">
                        <Bath className="h-4 w-4" aria-hidden />1 Bathroom
                      </span>
                      {room.sizeSqm && (
                        <span className="flex items-center gap-1">
