...
import { hospitalityServices, siteStats } from '@/data/mockData';
...
export function Footer() {
  ...
  const popularLinks = [
    { label: t.footer?.helpSupport ?? 'Help & Support', href: '#' },
    { label: t.footer?.trustSafety ?? 'Trust & Safety', href: '#' },
    { label: "FAQ's", href: '#' },
    { label: t.footer?.featureRooms ?? 'Feature Rooms', href: '/rooms' },
    { label: t.footer?.hotelBooking ?? 'Hotel Booking', href: '/' },
  ];

  return (
    <footer className="bg-gray-900 text-white">
      {/* Upper: CTA + Newsletter */}
      ...
      <div className="mx-auto max-w-7xl px-4 py-12">
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            <h3 className="font-bold text-white">{t.footer?.services ?? 'Services'}</h3>
          ...
          <div>
            <h3 className="font-bold text-white">{t.footer?.popularLinks ?? 'Popular links'}</h3>
            <ul className="mt-4 space-y-2">
